...
PROJ = ROOT.parent
DARKIR = PROJ / "retinex_darkIR_submission"

for p in (ROOT, PROJ / "dataset", DARKIR):
    s = str(p)
...
_metrics = _load_module("darkir_metrics", DARKIR / "metrics.py")
_losses = _load_module("darkir_losses", DARKIR / "losses.py")
psnr = _metrics.psnr
LPIPSMetric = _metrics.LPIPSMetric
ssim_map = _losses.ssim_map
...
def load_nafnet(checkpoint: Path, device: torch.device, base_ch: int) -> NAFNet:
    model = NAFNet(
        base_ch=base_ch,
        enc_blocks=[2, 2, 4, 8],
        dec_blocks=[2, 2, 2, 2],
    ...
    if isinstance(ckpt, dict) and ("ema" in ckpt or "model" in ckpt):
        ...
        state = ckpt
    model.load_state_dict(_strip_compile_prefix(state), strict=True)
    return model.eval()
...
def run_val(
    ...
    model = load_nafnet(checkpoint, device, base_ch)
    ...
    for i, (low, high) in enumerate(tqdm(loader, desc="calc", unit="img")):
        low = low.to(device)
        high = high.to(device)
        pred = torch.clamp(model(low), 0.0, 1.0).float()
        high = high.float()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="NAFNet grading calculator")
    parser.add_argument("--checkpoint", default="checkpoints/aminbagus.pth")
    ...
    raw = run_val(ckpt, val_dir.resolve(), device, args.base_ch, args.max_images)
    ranks = {
